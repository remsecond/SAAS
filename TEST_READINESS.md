...
## Change after first publish — in-app switcher removed (September 18, 2026)

Roberto's direction after seeing the beta: once a profile is open, nobody switches. The Max/Adrian bar and the "All profiles" chip are gone from every screen. A profile is chosen on the profile screen and remembered on that phone. One underlined "Not Max? Change profile" link sits at the very bottom of Home, and a "Change profile" button appears on the "not available" screen, so nobody can get stuck in the wrong profile. Per-profile drafts/checklists, the stale-response guard and server-side separation are unchanged. Removal only; no new feature.

Checks: automated suite 25/25 (tests updated to assert there is no switcher and that Change profile returns and forgets). Real browser (cloud Chromium, real bundles) 28/28 at 320 and 390 px: each card opens its own coursework and names the student; no switcher or other-profile control on Home or detail pages; none of the other student's courses; no overflow; Change profile is 44 px tall, keyboard-reachable with a visible ring, returns to the profile screen and forgets the choice; reload goes straight back into the remembered profile.
